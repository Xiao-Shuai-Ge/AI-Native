...
阶段输出：LangGraph/CrewAI 双模式与自动选择能力。
...
- 完成三个最小页面和路由。
- 对接创建、详情、列表、设置 API。
- 新建任务页支持自动/LangGraph/CrewAI 和模型供应商选择。
- 设置页支持在线修改三个 Agent 的 role、goal、backstory/instructions、temperature、max_tokens。
- 使用 SSE 实时更新任务状态。
- 展示实际引擎、自动选择原因、Agent 时间线、工具调用、Token、错误和最终 Markdown 报告。
- 补充加载、空数据、断线和失败状态。

当天验收：

- 不通过 Swagger，也能在浏览器完成完整任务。
- 页面能够分别提交 LangGraph、CrewAI 和 auto 任务。
- 刷新任务详情页后状态不会丢失。
...
- 完整 Compose 编排 API、worker、MCP、Web、Dapr sidecar、Redis、PostgreSQL、Jaeger、Prometheus。
- 执行单元、集成和端到端检查。
...
| 第 8 天仍未形成完整闭环 | 冻结新功能，先完成 UI、Compose、README 和恢复演示 |
